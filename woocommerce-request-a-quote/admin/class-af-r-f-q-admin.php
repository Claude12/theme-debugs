...
if ( ! class_exists( 'AF_R_F_Q_Admin' ) ) {
	...
	class AF_R_F_Q_Admin extends Addify_Request_For_Quote {
		...
		public function __construct() {
			...
			//adding pdf download action in quote list
			add_filter( 'post_row_actions', array( $this, 'af_rfq_post_link_genrate_pdf' ), 10, 2 );
			add_action('wp_ajax_download_quote_pdf', array( $this, 'af_rfq_download_pdf_by_post_link' ));
			...
			// add default fields in rule.
			add_action( 'init', array( $this, 'af_r_rf_q_create_field_rule' ) );

			add_action( 'admin_notices', array( $this, 'afrfq_display_error_message' ) );

			add_filter( 'redirect_post_location', array( $this, 'afrfq_handle_post_saving_message' ), 10, 2 );
		}


		public function afrfq_display_error_message() {
			$message = get_transient( 'afrfq_quote_save_error_' . get_current_user_id() );
			if ( $message ) {
				?>
				...
				<?php
				delete_transient( 'afrfq_quote_save_error_' . get_current_user_id() );
			}
		}

		public function afrfq_handle_post_saving_message( $location, $post_id ) {

			if ( get_transient( 'afrfq_quote_save_error_' . get_current_user_id() ) ) {
				$location = remove_query_arg( array( 'message', 'updated', 'revision' ), $location );
			}
		
			return $location;
		}

		public function af_r_rf_q_create_field_rule() {
			...
			if ( empty( $existing_posts ) ) {
				...
				foreach ( $default_field_posts as $index => $field_post ) {
					$post_id = wp_insert_post( $field_post );
					if ( !is_wp_error( $post_id ) ) {
						switch ( $index ) {
							case 0:
								update_post_meta( $post_id, 'afrfq_field_name', 'afrfq_field_' . $post_id );
								update_post_meta( $post_id, 'afrfq_field_label', 'First Name' );
								update_post_meta( $post_id, 'afrfq_field_type', 'text' );
								update_post_meta( $post_id, 'afrfq_field_placeholder', 'First Name' );
								update_post_meta( $post_id, 'afrfq_field_width', 'half_width' );
								update_post_meta( $post_id, 'afrfq_field_enable', 'enable' );
								update_post_meta( $post_id, 'afrfq_field_required', 'yes' );
								$post_data = array(
									'ID'         => $post_id,
									'menu_order' => $index,
								);
							...
							case 1:
								update_post_meta( $post_id, 'afrfq_field_name', 'afrfq_field_' . $post_id );
								update_post_meta( $post_id, 'afrfq_field_label', 'Last Name' );
								update_post_meta( $post_id, 'afrfq_field_type', 'text' );
								update_post_meta( $post_id, 'afrfq_field_width', 'half_width' );
								update_post_meta( $post_id, 'afrfq_field_placeholder', 'Last Name' );
								update_post_meta( $post_id, 'afrfq_field_enable', 'enable' );
								update_post_meta( $post_id, 'afrfq_field_required', 'yes' );
								$post_data = array(
									'ID'         => $post_id,
							...
							case 2:
								update_post_meta( $post_id, 'afrfq_field_name', 'afrfq_field_' . $post_id );
								update_post_meta( $post_id, 'afrfq_field_label', 'Email' );
								update_post_meta( $post_id, 'afrfq_field_type', 'email' );
								update_post_meta( $post_id, 'afrfq_field_width', 'full_width' );
								update_post_meta( $post_id, 'afrfq_field_placeholder', 'Email' );
								update_post_meta( $post_id, 'afrfq_field_enable', 'enable' );
								update_post_meta( $post_id, 'afrfq_field_required', 'yes' );
								$post_data = array(
									'ID'         => $post_id,
		...
		public function af_r_f_q_setting_files() {
			...
			include_once AFRFQ_PLUGIN_DIR . '/admin/settings/tabs/convert-to-cart-restriction.php';
			...
			include_once AFRFQ_PLUGIN_DIR . '/admin/settings/tabs/popup-settings.php';
			include_once AFRFQ_PLUGIN_DIR . '/admin/settings/tabs/quote-btn-shortcode.php';
		...
		public function af_r_f_q_variable_fields( $loop, $variation_data, $variation ) {
			...
			wp_nonce_field( 'afrfq_fields_nonce_action', 'afrfq_field_nonce' );
		...
		public function afrfq_update_fields_meta( $post_id ) {
			...
			if ( in_array( get_post_status( $post_id ), $exclude_statuses ) || 'untrash' === $action ) {
				return;
			}
		...
		public function afrfq_update_quote_details( $post_id ) {
			...
			// check first if this quote is already converted to cart
			$cart_link_for_converted_quote = get_post_meta( $post_id, 'afrfq_convert_to_cart_url', true );
			if ( '' === $cart_link_for_converted_quote && isset( $form_data['quote_status'] ) && 'af_converted_to_cart' === $form_data['quote_status'] ) {
				include_once AFRFQ_PLUGIN_DIR . 'admin/meta-boxes/quotes/update-quote.php';
				if ( get_transient( 'afrfq_quote_save_error_' . get_current_user_id() ) ) {
					return;
				}
				include_once AFRFQ_PLUGIN_DIR . 'admin/meta-boxes/quotes/convert-quote-to-cart.php';
			}

			// check first if this quote is already converted to order
			$order_id_for_converted_quote = get_post_meta( $post_id, 'order_for_this_quote', true );
			if ( '' === $order_id_for_converted_quote && isset( $form_data['quote_status'] ) && 'af_converted' === $form_data['quote_status'] ) {
				include_once AFRFQ_PLUGIN_DIR . 'admin/meta-boxes/quotes/update-quote.php';
				if ( get_transient( 'afrfq_quote_save_error_' . get_current_user_id() ) ) {
					return;
				}
			...
			if ( isset( $form_data['addify_convert_to_cart'] ) || isset( $form_data['afrfq_regenerate_cart_url'] ) ) {
				include_once AFRFQ_PLUGIN_DIR . 'admin/meta-boxes/quotes/update-quote.php';
		...
		public function afrfq_admin_scripts() {
			...
			// if ( $screen && in_array( $screen->id, $this->get_tab_screen_ids(), true ) ) {
				...
				// wp_enqueue_style( 'afrfq-fontawesome', AFRFQ_URL . '/assets/external-dependencies/fontawesome-free-6.6.0-web/css/all.min.css', array(), '6.6.0' );
		...
		public function afrfq_add_metaboxes() {
			...
			add_meta_box(
				'afrfq-quote-notes',
				esc_html__( 'Quote Notes', 'addify_b2b' ),
				array( $this, 'afrfq_quote_notes_callback' ),
				'addify_quote',
				'side',
				'low'
			);
		 ...
		 * AF_R_F_Q_Admin.
		 */
		public function afrfq_quote_notes_callback() {
			include_once AFRFQ_PLUGIN_DIR . 'admin/meta-boxes/quotes/quote-notes.php';
		}
		...
		public function afrfq_rule_setting_callback() {
			...
			$afrfq_hide_brands        = unserialize( get_post_meta( intval( $post->ID ), 'afrfq_hide_brands', true ) );
		...
		// $actions ['af_rfq_generate_pdf'] = '<button type="submit" id="af_rfq_download_pdf_with_qoute_id_admin"  name="af_rfq_download_pdf_with_qoute_id_admin" value="' . $hashed_quote_id . '" class="woocommerce-button fas fa-file-pdf af_rfq_download_pdf_with_qoute_id_admin">PDF</button>';
		...
		public function addify_quote_columns_content( $column_name, $post_ID ) {
			...
			switch ( $column_name ) {
				...
				case 'email':
					...
					break;
				case 'status':
					$status = get_post_meta( $post_ID, 'quote_status', true );
					$status = str_replace('af_', '', $status); // Remove af_ prefix
					$status = str_replace('_', ' ', $status); // Replace underscores with spaces
					$status = 'converted' === $status ? 'Converted to Order' : $status;
					echo esc_attr( ucwords($status) );
					break;
			}
		}
		...
		public function af_rfq_post_link_genrate_pdf( $actions, WP_Post $post ) {
			...
			$hashed_quote_id= base64_encode( $quote_id . '|' . hash_hmac( 'sha256', $quote_id, wp_salt( 'auth' ) ) );


			$download_url = wp_nonce_url(admin_url('admin-ajax.php?action=download_quote_pdf&quote_id=' . $hashed_quote_id), 'afquote-ajax-nonce');
		   
			$actions['af_rfq_generate_pdf'] = '<a href="' . esc_url($download_url) . '"><i class="fas fa-file-pdf"></i> PDF</a>';
		...
		public function af_rfq_download_pdf_by_post_link() {
			...
			if (!wp_verify_nonce($nonce, 'afquote-ajax-nonce')) {
				wp_die(esc_html__('Security check failed', 'addify_b2b'));
			}
		...
		public function af_rfq_apply_download_pdf_action() {
			...
			if ( 'download_pdf_action' == $afrfq_is_action_pdf_bulk_download ) {
				...
				if ( ! empty( $selected_posts ) ) {
					af_rfq_gernate_pdf_of_order( false, false, false, true, false, false, false, $selected_posts );
				}
