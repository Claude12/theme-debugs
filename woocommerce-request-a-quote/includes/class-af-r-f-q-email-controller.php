...
class AF_R_F_Q_Email_Controller {
	...
	public function __construct() {
		...
		$this->init();
		add_action( 'addify_rfq_woocommerce_email_header', array( $this, 'get_email_header' ) );
		add_action( 'addify_rfq_woocommerce_email_footer', array( $this, 'get_email_footer' ) );
		add_action( 'addify_rfq_email_customer_details', array( $this, 'get_customer_info_table' ) );
		add_action( 'addify_rfq_email_quote_details', array( $this, 'get_quote_contents_table' ), 10, 3 );
	...
	public function send_email_to_admins( $quote_id ) {
		...
		} elseif ( 'af_converted_to_cart' === $quote_status ) {

			$email_enable  = isset( $email_values['af_admin_converted_to_cart']['enable'] ) ? $email_values['af_admin_converted_to_cart']['enable'] : '';
			$email_subject = isset( $email_values['af_admin_converted_to_cart']['subject'] ) ? $email_values['af_admin_converted_to_cart']['subject'] : '';
			$email_heading = isset( $email_values['af_admin_converted_to_cart']['heading'] ) ? $email_values['af_admin_converted_to_cart']['heading'] : '';
			$email_message = isset( $email_values['af_admin_converted_to_cart']['message'] ) ? $email_values['af_admin_converted_to_cart']['message'] : '';
	...
	public function style_inline( $content ) {
		...
		if ( class_exists( $emogrifier_class ) ) {
			try {
				...
				$html_prune->disableStyleBlocksParsing();
			...
			$content = '<head><style>' . $css . '</style></head>' . $content;
		}
	...
	private function get_store_address() {
		...
		if ( strpos( $store_country, ':' ) !== false ) {
			list( $store_country, $store_state ) = explode( ':', $store_country );

			if ( isset( WC()->countries->countries[ $store_country ] ) ) {
				$country_name = WC()->countries->countries[ $store_country ];
			} else {
				$country_name = $store_country;
			}

			if ( isset( WC()->countries->states[ $store_country ][ $store_state ] ) ) {
				$state_name = WC()->countries->states[ $store_country ][ $store_state ];
			} else {
				$state_name = $store_state;
			}
		} elseif ( isset( WC()->countries->countries[ $store_country ] ) ) {
				$country_name = WC()->countries->countries[ $store_country ];
		} else {
			$country_name = $store_country;
		}
		...
		if ( ! empty( $store_address ) ) {
			$address_parts[] = $store_address;
		}
		if ( ! empty( $store_address_2 ) ) {
			$address_parts[] = $store_address_2;
		}
		if ( ! empty( $store_city ) ) {
			$address_parts[] = $store_city;
		}
		if ( ! empty( $store_state ) ) {
			$address_parts[] = $state_name;
		}
		if ( ! empty( $store_postcode ) ) {
			$address_parts[] = $store_postcode;
		}
	 ...
	 * @param object $quote_id returns qoute id.
	 */
	public function get_quote_contents_table( $quote_id, $email, $email_type ) {


		$quote_contents             = get_post_meta( $quote_id, 'quote_contents', true );
		$quote_status               = get_post_meta( $quote_id, 'quote_status', true );
		$afrfq_convert_to_cart_url  = get_post_meta( $quote_id, 'afrfq_convert_to_cart_url', true );

		if ( ! isset( $af_quote ) ) {
			$af_quote = new AF_R_F_Q_Quote( $quote_contents );
		}

		$shipping_cost = get_post_meta( $quote_id, 'afrfq_shipping_cost', true );
		...
		$offered_vat    = isset( $totals['_offered_tax_total'] ) ? $totals['_offered_tax_total'] : 0;
		$quote_offered_total_after_tax = isset( $totals['_offered_total_after_tax'] ) ? $totals['_offered_total_after_tax'] : 0;
		...
		wc_get_template(
			...
			array(
				...
				'offered_vat'               => $offered_vat,
				'quote_offered_total_after_tax' => $quote_offered_total_after_tax,
				'shipping_cost'             => $shipping_cost,
				'colspan'                   => $colspan,
				'quote_status'              => $quote_status,
				'afrfq_convert_to_cart_url' => $afrfq_convert_to_cart_url,
	...
	public function get_quote_user_info( $quote_id ) {
		...
		$customer_info['quote_id']   = array(
			'label' => __( 'Quote Number', 'addify_b2b' ),
			'value' => $quote_id,
		);
		$customer_info['quote_date'] = array(
			'label' => __( 'Quote Date', 'addify_b2b' ),
			'value' => $quote_date,
		);
		...
		foreach ( $quote_fields as $key => $field ) {
			...
			if ( 'file' == $afrfq_field_type && ! empty( $field_data ) ) {
				...
				$field_data = sprintf( '<a class="button" style="%s" href="%s">%s</a>', esc_attr( $style ), esc_url( AFRFQ_UPLOAD_URL . $field_data ), esc_html__( 'View File', 'addify_b2b' ) );
			}
