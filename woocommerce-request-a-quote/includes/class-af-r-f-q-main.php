...
if ( ! class_exists( 'AF_R_F_Q_Main' ) ) {
	...
	class AF_R_F_Q_Main {
		...
		public function afrfq_update_quote_data_after_login( $user_login, $user ) {
			...
			if ( ! empty( $final_quotes ) && isset( WC()->session ) && WC()->session instanceof WC_Session ) {
			...
			foreach ( $this->quote_rules as $rule ) {
				...
				if ( $this->check_required_addons( $product->get_id() ) ) {
					...
					return apply_filters( 'addons_add_to_cart_text', __( 'Select options', 'woocommerce-product-addons' ) );
						...
						$quote_button = true;
						echo '<a href="javascript:void(0)" rel="nofollow" data-rule_id="' . esc_attr($rule->ID) . '" data-product_id="' . intval( $product->get_ID() ) . '" data-product_sku="' . esc_attr( $product->get_sku() ) . '" class="afrfqbt button add_to_cart_button product_type_' . esc_attr( $product->get_type() ) . '">' . esc_attr( $afrfq_custom_button_text ) . '</a>';
		...
		public function afrfq_custom_button_add_replacement() {
			...
			foreach ( $this->quote_rules as $rule ) {
				...
				if ( 'addnewbutton' === $afrfq_is_hide_addtocart || 'replace' === $afrfq_is_hide_addtocart ) {
					$quote_button = true;
					echo '<a href="javascript:void(0)" rel="nofollow" data-rule_id="' . esc_attr($rule->ID) . '" data-product_id="' . intval( $product->get_ID() ) . '" data-product_sku="' . esc_attr( $product->get_sku() ) . '" class="afrfqbt_single_page  single_add_to_cart_button button alt  ' . esc_attr( $disable_class ) . '   product_type_' . esc_attr( $product->get_type() ) . '">' . esc_attr( $afrfq_custom_button_text ) . '</a>';
					array_push( self::$rule_applied, $rule->ID );
