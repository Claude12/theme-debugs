...
class AF_R_F_Q_Quote {
	...
	public function calculate_totals() {
		...
		foreach ( (array) $this->quote_contents as $quote_item_key => $quote_item ) {
			...
			} elseif (!isset($quote_item['composite_child_products']) || empty($quote_item['composite_child_products'])) {
	...
	public function get_calculated_totals( $contents = array(), $quote_id = 0 ) {
		...
		// the key '_offered_total' is actually subtotal data(excluding tax and shipping)
		$quote_totals = array(
			'_subtotal'                 => 0,
			'_offered_total'            => 0,
			'_tax_total'                => 0,
			'_offered_tax_total'        => 0,
			'_shipping_total'           => floatval( $_shipping_cost ),
			'_total'                    => 0,
			'_offered_total_after_tax'  => 0,
		);
		...
		foreach ( $contents as $quote_item_key => $quote_item ) {
			...
			if ( $product->is_taxable() ) {
				...
				if ( ! wc_prices_include_tax() ) {
					...
					$offered_product_subtotal = wc_get_price_including_tax(
						$product,
						array(
							'qty'   => $quantity,
							'price' => $offered_price,
				...
				$difference_price = ( floatval( $price ) * intval( $quantity ) ) - floatval( $product_subtotal );
				$offered_difference_price = ( floatval( $offered_price ) * intval( $quantity ) ) - floatval( $offered_product_subtotal );

				if ( $difference_price < 0 ) {

					$difference_price = $difference_price * -1;
				}
				if ($offered_difference_price < 0) {
					$offered_difference_price = $offered_difference_price * -1;
				}
				if (!isset($quote_item['composite_child_products']) || empty($quote_item['composite_child_products'])) {
					$quote_totals['_subtotal']  += $price * $quantity;
					$quote_totals['_tax_total'] += $difference_price;
					$quote_totals['_offered_tax_total'] += $offered_difference_price;
				}
					...
					$quote_totals['_offered_tax_total'] += 0;
			}
		}

		if ( 'yes' == get_option( 'woocommerce_prices_include_tax' ) ) {
			$quote_totals['_total'] = $quote_totals['_subtotal'] + $quote_totals['_shipping_total'];
			$quote_totals['_offered_total_after_tax'] = $quote_totals['_offered_total']  + $quote_totals['_shipping_total'];
		} else {
			$quote_totals['_total'] = $quote_totals['_subtotal'] + $quote_totals['_tax_total'] + $quote_totals['_shipping_total'];
			$quote_totals['_offered_total_after_tax'] = $quote_totals['_offered_total'] + $quote_totals['_offered_tax_total'] + $quote_totals['_shipping_total'];
		}
	...
	public function add_to_quote( $form_data, $product_id = 0, $quantity = 1, $variation_id = 0, $variation = array(), $quote_item_data = array(), $return_contents = false ) {
		try {
			...
			$applied_rule_id = isset( $form_data['afrfq_rule_id'] ) ? $form_data['afrfq_rule_id'] : 0;
			...
			if ( $product_data->is_type( 'variation' ) ) {
				...
				if ( empty( $variation_id ) ) {
					throw new Exception( __( 'Please choose product options&hellip;', 'addify_b2b' ) );
				}

				// Do we have a variation ID?
				if ( ! $this->check_quote_availability_for_variation( $variation_id ) ) {
					throw new Exception( __( 'Quote is not permitted for selected variation &hellip;', 'addify_b2b' ) );
				}
				...
				foreach ( $parent_data->get_attributes() as $attribute ) {
					...
					if ( isset( $posted_and_variation_attributes[ $attribute_key ] ) ) {
						...
						if ( $valid_value === $value ) {
							...
							throw new Exception( sprintf( __( 'Invalid value posted for %s', 'addify_b2b' ), wc_attribute_label( $attribute['name'] ) ) );
						}
					} elseif ( '' === $valid_value ) {
				...
				if ( ! empty( $missing_attributes ) && ! is_admin() ) {
					/* translators: %s: Attribute name. */
					throw new Exception( sprintf( _n( '%s is a required field', '%s are required fields', count( $missing_attributes ), 'addify_b2b' ), wc_format_list_of_items( $missing_attributes ) ) );
				}
			}
			...
			if ( $product_data->is_sold_individually() ) {
				...
				if ( $found_in_quote ) {
					...
					$message = sprintf( __( 'You cannot add another "%s" to your quote.', 'addify_b2b' ), $product_data->get_name() );
					...
					throw new Exception( sprintf( '<a href="%s" class="button wc-forward">%s</a> %s', '', __( 'View Quote', 'addify_b2b' ), $message ) );
				}
			}

			if ( ! $product_data->is_purchasable() ) {
				$message = __( 'Sorry, this product cannot be purchased.', 'addify_b2b' );
				/**
			...
			if ( ! empty( $quote_item_key ) ) {
				...
				// handling composite product -- updating the parent quanity count in child meta
				if (isset($this->quote_contents[ $quote_item_key ]['composite_child_products']) && !empty($this->quote_contents[ $quote_item_key ]['composite_child_products'])) {
					$parent_item_key = $this->quote_contents[ $quote_item_key ]['composite_child_products']['parent_id'];
					$parent_quantity = $this->quote_contents[ $parent_item_key ]['quantity'];
					$this->quote_contents[ $quote_item_key ]['composite_child_products']['parent_quantity'] = $parent_quantity;
				}
			} else {
		...
		if ( is_array( $quote_item_data ) && ! empty( $quote_item_data ) ) {
			...
			foreach ( $quote_item_data as $key => $value ) {
				if ( is_array( $value ) || is_object( $value ) ) {
					// this for addify composite product builder -- to remove parent quantity from quote id
					if (isset($value['parent_quantity'])) {
						unset($value['parent_quantity']);
					}
	...
	public function convert_quote_to_order( $quote_id = false ) {
		...
		if ( false === $quote_id ) {
			wc_add_notice( __( 'Quote ID is required to convert a quote to order.', 'addify_b2b' ), 'error' );
			return false;
		}
		...
		if ( empty( $quote_contents ) ) {
			wc_add_notice( __( 'Quote Contents are empty.', 'addify_b2b' ), 'error' );
			return false;
		}

		$quotes      = $quote_contents;
		$quote_order = wc_create_order();
		$contain_invalid_product = false;
		...
		$shipping_cost = floatval( get_post_meta( $quote_id, 'afrfq_shipping_cost', true ) );
		if ( $shipping_cost ) {
		...
		update_post_meta( $quote_id, 'order_for_this_quote', $quote_order->get_id() );
		...
		wc_add_notice(
			...
			__( 'Your Quote# %1$s has been converted to Order# %2$s.', 'addify_b2b' ),
			$quote_id,
			$quote_order->get_id()
			),
			'success'
		...
		$pay_link = $quote_order->get_checkout_payment_url();

		wp_safe_redirect( $pay_link );
		exit;
	}
	 ...
	 * @param mixed $quote_id id of quote to convert.
	 */
	public function convert_quote_to_cart( $quote_id = false ) {

		if ( false === $quote_id ) {
			wc_add_notice( __( 'Quote ID is required to convert a quote to cart.', 'addify_b2b' ), 'error' );
			return false;
		}
		...
		// Build the link (adjust the endpoint as needed)
		$convert_to_cart_url = add_query_arg(
			...
			home_url( '/' ) // You can change this to a custom endpoint if needed
		);
		...
		update_post_meta( $quote_id, 'quote_status', 'af_converted_to_cart' );

		do_action( 'addify_rfq_send_quote_email_to_admin', $quote_id );

		do_action( 'addify_rfq_send_quote_email_to_customer', $quote_id );

		wp_safe_redirect( $convert_to_cart_url );
		exit;
	}
	...
	public function insert_new_quote( $post_data = array() ) {
		...
		if ( empty( $post_data ) || ! is_array( $post_data ) ) {
			if ( ! is_admin() ) {
				wc_add_notice( __( 'Post data should not be empty to create a quote.', 'addify_b2b' ), 'error' );
			}
			return;
		}
		...
		if ( empty( wc()->session->get( 'quotes' ) ) ) {
			if ( ! is_admin() ) {
				wc_add_notice( __( 'No item found in quote basket.', 'addify_b2b' ), 'error' );
			}
			return;
		}
		...
		try {
			...
			add_action( 'addify_rfg_success_message', array( $this, 'addify_rfg_success_message_callback' ));

			$is_popup_submission = isset($post_data['popup_template']) && '' != $post_data['popup_template'];

			if ( 'yes' === get_option( 'afrfq_redirect_after_submission' ) && ! empty( get_option( 'afrfq_redirect_url' ) ) && !$is_popup_submission ) {
				// for redirecting to custom url.Removed the wp_safe_redirect
	...
	public function addify_rfg_success_message_callback() {
		...
		$afrfq_success_message = '<div class="afrfq_success_message">' . esc_html__( $afrfqmsg, 'addify_b2b' ) . '</div>';

		echo wp_kses_post($afrfq_success_message);
	}
}