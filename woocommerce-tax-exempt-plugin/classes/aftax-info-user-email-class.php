...
if ( !class_exists( 'Addify_Tax_Info_User_Email' ) ) { 

	class Addify_Tax_Info_User_Email extends WC_Email {
		...
		public function __construct() { 
			$this->id             = 'aftax_info_user_email'; // Unique ID to Store Emails Settings
			$this->title          = __( 'Addify Tax Information Email to Customer', 'addify_b2b' ); // Title of email to show in Settings
			$this->customer_email = true; // Set true for customer email and false for admin email.
			$this->description    = __( 'This email will be sent to customer when tax information is added or updated by the customer.', 'addify_b2b' ); // description of email
			$this->template_base  = AFTAX_PLUGIN_DIR . 'templates/'; // Base directory of template 
			$this->template_html  = 'emails/aftax-info-email-user.php'; // HTML template path
			$this->template_plain = 'emails/plain/aftax-info-email-user.php'; // Plain template path
			...
			// Trigger function.
			add_action( 'aftax_info_notification_user', array( $this, 'trigger' ), 10, 4 ); // action hook(s) to trigger email 
		}
		...
		public function get_default_subject() {
			...
			return __( 'Tax Exemption Request Submitted Successfully', 'addify_b2b' );
		}
		...
		public function trigger( $customer_id, $tax_field, $taxarea_field, $file_field ) {
			...
			$this->setup_locale();
				...
				//custom message
				$email_content = get_option('aftax_customer_email_message');
