...
if ( !class_exists( 'Addify_Tax_Expire_Info_Admin_Email' ) ) { 

	class Addify_Tax_Expire_Info_Admin_Email extends WC_Email {
		...
		public function __construct() { 
			$this->id             = 'aftax_expire_info_admin_email'; // Unique ID to Store Emails Settings
			$this->title          = __( 'Addify Expire Tax Information Email to Admin', 'addify_b2b' ); // Title of email to show in Settings
			$this->customer_email = false; // Set true for customer email and false for admin email.
			$this->description    = __( 'This email will be sent to admin when tax information is expired.', 'addify_b2b' ); // description of email
			$this->template_base  = AFTAX_PLUGIN_DIR . 'templates/'; // Base directory of template 
			$this->template_html  = 'emails/aftax-expire-info-email-admin.php'; // HTML template path
			$this->template_plain = 'emails/plain/aftax-expire-info-email-admin.php'; // Plain template path
			...
			// Trigger function.
			add_action( 'aftax_expire_info_notification_admin', array( $this, 'trigger' ), 10, 4 ); // action hook(s) to trigger email 
		}
		...
		public function trigger( $customer_id, $tax_field, $taxarea_field, $file_field ) {
			...
			$this->setup_locale();
				...
				//custom message
				$email_content = get_option('aftax_expire_tax_info_email_message');
