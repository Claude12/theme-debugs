...
class Frontend_Scripts implements Standard_Service, Registerable, Conditional {
	...
	public function register_assets(): void {
		...
		$script_params = [
			...
			'create_thumbnails'        => Util::shall_create_upload_thumbnails(),
			'tax_info'                 => new \WC_Tax(),
			'tax_conflicts'            => Price_Util::has_conflicting_tax_settings(),
			'addons_use_cart_quantity' => Cart_Util::shall_addons_use_cart_quantity( $product ),
			'show_one_error_per_field' => Util::show_one_error_per_field(),
			'show_invalid_form_total'  => Util::show_invalid_form_total(),
		];
	...
	public function enqueue_assets(): void {
		...
		if (
			is_cart() ||
			is_checkout() ||
			WC_Blocks_Utils::has_block_in_page( get_post(), 'woocommerce/cart' ) ||
			WC_Blocks_Utils::has_block_in_page( get_post(), 'woocommerce/checkout' )
		) {
