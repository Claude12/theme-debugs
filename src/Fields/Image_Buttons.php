...
class Image_Buttons extends Checkboxes {
	...
	private function render_image_buttons(): void {
		...
		foreach ( $this->get_choices() as $index => $choice ) {
			...
			$html           .= sprintf(
				...
				$this->get_choice_quantity_picker_html( $index )
			);
		}
	...
	private function get_figcaption( $index ) {
		$empty_placeholder = '<span class="wpo-empty-placeholder">&nbsp;</span>';
		...
		$label  = $display_label || $this->is_any_image_missing ? $this->get_label( $index ) : '';
		$label  = $label ? sprintf( '<span class="wpo-image-label">%1$s</span>', $label ) : $empty_placeholder;
		...
		return sprintf(
			...
			$label,
			$this->choices_have_equal_pricing() ? '' : $price
		);
	}
