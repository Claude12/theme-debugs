...
use Barn2\Plugin\WC_Product_Options\Fields\Products;
use Exception;
...
trait Cart_Item_Data_Formula {
	 ...
	 * @param int $quantity
	 * @param array $options
	 * @param int|null $qty_picker
	 * @return array
	 */
	public function get_cart_item_data( $value, $product, $quantity, $options, $qty_picker = null ): ?array {
	...
	private function get_option_variable_value( object $option, array $options, array $formula_variable, $quantity ): ?float {
		...
		if ( ! is_string( $sanitized_value ) && ! is_numeric( $sanitized_value ) ) {
			return null;
		}
	...
	/**
	 * Process the value of a variable based on its type and property chain.
	 *
	 * @param mixed $value The value to be processed.
	 * @param object $option The option object associated with the value.
	 * @param array $formula_variable The formula variable details.
	 * @param array $options The array of all options.
	 * @param int $quantity The quantity of the product.
	 * @return float|string|null The processed value, or null if it cannot be processed.
	 */
	private function process_value( $value, $option, $formula_variable, $options, $quantity ) {
		...
		switch ( $option->type ) {
			...
			case 'color_swatches':
			case 'text_labels':
				$value = $this->process_choice_value( $value, $option, $formula_variable, $options );
				break;
			...
			case 'product':
				$product_field = new Products( $option, $this->product );
				$products      = array_filter(
					wc_get_products( $product_field->get_product_type_args() ),
					function ( $product ) {
						return $product->is_purchasable() && $product->is_in_stock();
					}
				);

				$choice_count = count( $products );

				if ( $product_field->get_setting( 'product_selection' ) === 'manual' ) {
					$choice_count = array_reduce(
						$product_field->get_setting( 'manual_products' ),
						function ( $total_products, $product ) {
							if ( is_array( $product['variations'] ) ) {
								$total_products += count( $product['variations'] );
							} else {
								++$total_products;
							}
				...
				// get the count of possible choices
				...
				switch ( $prop_chain[1] ) {
					case 'count':
						...
						break;
					case 'all':
						$value = ( count( $value ) === $choice_count ? 1 : 0 );
						break;
					case 'any':
	...
	private function process_choice_value( $value, $option, $formula_variable, $options ) {
		if ( ! is_array( $value ) ) {
			$value = [ $value ];
		}
		...
		switch ( $prop_chain[1] ) {
			...
			case 'totalqty':
				...
				foreach ( $option->choices as $choice ) {
					if ( is_array( $value ) && in_array( $choice['id'], $value, true ) ) {
						$total_qty += intval( $options[ "option-{$option->id}-qty" ][ $choice['id'] ] ?? 0 );
					} elseif ( $choice['id'] === $value ) {
						$total_qty += intval( $options[ "option-{$option->id}-qty" ][ $choice['id'] ] ?? 0 );
					}
				}
				$value = $total_qty;
			...
			case 'quantity':
				$value = intval( $options[ "option-{$option->id}-qty" ] ?? 0 );
				break;
			...
			case 'choices':
				...
				if ( $prop_chain[3] === 'checked' ) {
					...
					$value = empty( $choice ) ? 0 : ( $choice[0]['value'] ?? 0 );
				} elseif ( $prop_chain[3] === 'quantity' ) {
					$qty = $options[ "option-{$option->id}-qty" ] ?? 0;

					if ( is_array( $options[ "option-{$option->id}-qty" ] ) && isset( $choice[0]['id'] ) ) {
						$qty = $options[ "option-{$option->id}-qty" ][ $choice[0]['id'] ] ?? 0;
					}

					$value = empty( $choice ) ? 0 : $qty;
				}
				break;
		}
