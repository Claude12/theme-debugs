...
class Color_Swatches extends Abstract_Field {
	...
	private function render_swatches(): void {
		$html = '<ul class="wpo-color-checkboxes wpo-choice-list">';

		foreach ( $this->get_choices() as $index => $choice ) {
			$choice['index'] = $index;
			$html           .= sprintf(
				'<li class="wpo-choice-item %9$s">
					<label aria-label="%10$s" title="%10$s">
				...
				strip_tags( parent::get_label( $index ) ),
	...
	public function get_label( $index = 0, $skip_filters = false ): string {
		if ( $this->display_label ) {
			return parent::get_label( $index, $skip_filters );
		}
