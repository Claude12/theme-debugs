...
use Barn2\Plugin\WC_Product_Options\Schema\Checkout_Links as Checkout_Links_Schema;
...
class Version_Updater implements Registerable, Standard_Service {
	 ...
	 * Add user_roles and exclude_user_roles columns to the groups table.
	 *
	 * @return void
	 */
	public function update_261(): void {
		global $wpdb;
		$update_version = '2.6.1';
		$table_name     = $wpdb->prefix . 'wpo_groups';
		...
		$exclude_user_roles_exists = $wpdb->get_results(
			$wpdb->prepare(
				'SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = %s AND TABLE_NAME = %s AND COLUMN_NAME = %s',
				DB_NAME,
				$table_name,
		...
		$settings_exists = $wpdb->get_results(
			$wpdb->prepare(
				'SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = %s AND TABLE_NAME = %s AND COLUMN_NAME = %s',
				DB_NAME,
				$table_name,
		...
		// Add user_roles column if it doesn't exist
		if ( empty( $user_roles_exists ) ) {
			$wpdb->query( "ALTER TABLE {$table_name} ADD COLUMN user_roles JSON AFTER exclude_categories" );
			$wpdb->query( "UPDATE {$table_name} SET user_roles = '[]'" );
		}

		// Add exclude_user_roles column if it doesn't exist
		if ( empty( $exclude_user_roles_exists ) ) {
			$wpdb->query( "ALTER TABLE {$table_name} ADD COLUMN exclude_user_roles JSON AFTER user_roles" );
			$wpdb->query( "UPDATE {$table_name} SET exclude_user_roles = '[]'" );
		}

		if ( empty( $settings_exists ) ) {
			$wpdb->query( "ALTER TABLE {$table_name} ADD COLUMN settings JSON AFTER exclude_user_roles" );
			$wpdb->query( "UPDATE {$table_name} SET settings = '{}'" );
		}

		// Finally update the version option
		update_option( 'woocommerce_product_options_version', $update_version );
	}
	 ...
	 *
	 * Create the wpo_checkout_links table if not present.
	 *
	 * @return void
	 */
	public function update_262(): void {
		$update_version = '2.6.2';

		$checkout_links_table = new Checkout_Links_Schema();
		$checkout_links_table->register();
	 ...
	 *
	 * Fix the default value of the empty columns
	 * in case the previous updates did not run correctly
	 *
	 * @return void
	 */
	public function update_263(): void {
		global $wpdb;
		$update_version = '2.6.3';
		$table_name     = $wpdb->prefix . 'wpo_groups';

		// Re-run update 2.6.1 to ensure the DB includes the new columns
		$this->update_261();

		// Set default values for the new columns in case the previous update did not set them correctly
		$wpdb->query( "UPDATE {$table_name} SET user_roles = '[]' WHERE user_roles IS NULL OR user_roles = ''" );
		$wpdb->query( "UPDATE {$table_name} SET exclude_user_roles = '[]' WHERE exclude_user_roles IS NULL OR exclude_user_roles = ''" );
		$wpdb->query( "UPDATE {$table_name} SET settings = '{}' WHERE settings IS NULL OR settings = ''" );
