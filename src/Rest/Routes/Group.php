...
class Group extends Base_Route implements Route {
			...
			[
				'args' => [
					'search' => [
						'type'        => 'string',
						'required'    => false,
						'description' => __( 'Search term to filter user roles.', 'woocommerce-product-options' ),
					],
				],
				[
					'methods'             => WP_REST_Server::READABLE,
	...
	public function get_visibility_objects( $group ) {
		...
		$roles	    = array_merge( $group->user_roles ?? [], $group->exclude_user_roles ?? [] );
		...
		$roles = array_map(
			function ( $role_key ) {
				if ( $role_key === 'guest' ) {
					return [
						'key'  => 'guest',
						'name' => __( 'Guest', 'woocommerce-product-options' ),
					];
				}
	...
	public function search_for_products( $request ) {
		...
		$products = array_values( array_filter( $query->get_products() ) );

		/**
		 * Filter the list of products so that other plugins can add or remove products.
		 *
		 * @param array $products The list of all products.
	...
	/**
	 * Get a formatted list of user roles.
	 *
	 * @param WP_REST_Request $request
	 * @return WP_REST_Response|WP_Error
	 */
	public function get_user_roles( $request ) {
		...
		if ( empty( $all_roles ) ) {
			return new WP_Error( 'wpo-rest-user-roles', __( 'No user roles found', 'woocommerce-product-options' ) );
		}

		// Add 'guest' pseudo-role for logged-out users
		$all_roles = array_merge(
			[ 'guest' => __( 'Guest (Logged Out Users)', 'woocommerce-product-options' ) ],
			$all_roles
		);
		...
		foreach ( $all_roles as $role_id => $role_name ) {
			// If search term is provided, filter roles
			if ( ! empty( $search ) ) {
				if ( stripos( $role_name, $search ) === false && stripos( $role_id, $search ) === false ) {
					continue;
				}
			}
			...
			$formatted_roles[] = [
				'id'   => $role_id,
				'key'  => $role_id,
				'name' => $role_id === 'guest' ? $role_name : translate_user_role( $role_name ),
			];
		}
	...
	private function get_group_schema() {
		return [
			...
			'user_roles'         => [
				...
				'required'    => false,
				'description' => __( 'The roles the group is visible to.', 'woocommerce-product-options' ),
			],
			'exclude_user_roles' => [
				'type'        => 'array',
				'items'       => [
				...
				'required'    => false,
				'description' => __( 'The roles the group is not visible to.', 'woocommerce-product-options' ),
			],
			'settings'           => [
				'type'        => [ 'object', 'null' ],
				'required'    => false,
				'description' => __( 'Any specific extra settings for the group.', 'woocommerce-product-options' ),
			],
			'options'            => [
				'type'        => 'array',
				'required'    => false,
