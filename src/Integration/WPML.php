...
class WPML implements Registerable, Standard_Service, Conditional {
	...
	public function register() {
		...
		// Filter the product ID used to get option groups attached to a product
		add_filter( 'wc_product_options_get_product_id', [ $this, 'get_original_product_id' ] );
		add_filter( 'wc_product_options_searched_products', [ $this, 'filter_searched_products' ] );
	}
	...
	public function get_string_args( $reference_object, $context = 'group_name', $index = null ) {
		...
		if ( $reference_object instanceof Option_Model ) {
			$group = Util::get_product_group( $reference_object->group_id );
		}
	...
	/**
	 * Get the main product ID for a given product
	 *
	 * When a product is translated, WPML creates a new product with a different ID.
	 * This function retrieves the original product ID for the given product
	 * in the default language, so that option groups are correctly associated.
	 *
	 * @param int $product_id The translated product ID
	 * @return int            The original product ID
	 */
	public function get_original_product_id( $product_id ) {
		global $sitepress;
		$original_product_id = $sitepress->get_original_element_id( $product_id, 'post_product' );

		return $original_product_id ? $original_product_id : $product_id;
	}

	/**
	 * Filter the products resulting from a search to return only products in the default language
	 *
	 * @param array $products The products resulting from the search
	 * @return array          The filtered products
	 */
	public function filter_searched_products( $products ) {
		global $sitepress;
		...
		foreach ( $products as $product ) {
			$product_language = $sitepress->get_language_for_element( $product->get_id(), 'post_product' );
