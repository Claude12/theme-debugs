...
class Product_Table implements Standard_Service, Registerable {
	...
	/**
	 * Whether the integration service is already initialized.
	 *
	 * @var bool
	 */
	...
	public function register() {
		...
		if ( wp_doing_ajax() && $action === 'wcpt_load_products' ) {
			// Initialize immediately during AJAX request
			// if the action is to load products for WPT.
			$this->init();
		} else {
			// Add the initialization to the wc_product_table_args filter
			// if it is a regular page load.
			add_filter( 'wc_product_table_args', [ $this, 'init' ] );
		}
	}
	 ...
	 *
	 * This callback always returns the original args.
	 * It is used here only as an action to initialize the integration.
	 *
	...
	public function init( array $args = [] ) {
		...
		if ( ! $single_product_handler instanceof Handlers\Single_Product ) {
			return $args;
		}
