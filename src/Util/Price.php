...
final class Price {
	 ...
	 * @return float
	 */
	public static function calculate_option_cart_price( $choice_data, $product, $quantity = 1, $product_price = 0.0 ): float {
		$price_data = $choice_data['pricing'] ?? [];
		$qty_picker = $choice_data['quantity'] ?? 1;
		...
		switch ( $price_data['type'] ) {
			case 'flat_fee':
				...
				break;
			case 'percentage_inc':
				$option_price = $qty_picker * $product_price * ( $price_data['amount'] / 100 );
				break;
			case 'percentage_dec':
				$option_price = - ( $qty_picker * $product_price * ( $price_data['amount'] / 100 ) );
				break;
			case 'quantity_based':
				$option_price = $qty_picker * $price_data['amount'];
				break;
			case 'price_formula':
	...
	/**
	 * Replace several placeholders in a price template.
	 *
	 * @param string $price_template
	 * @param WC_Product $product
	 * @return string
	 */
	public static function format_price_template( string $price_template, WC_Product $product ): string {
		$placeholders = [
			'{price}'         => sprintf( '<span class="wpo-price">%s</span>', $product->get_price() ),
			'{regular_price}' => sprintf( '<span class="wpo-price">%s</span>', $product->get_regular_price() ),
			'{sale_price}'    => sprintf( '<span class="wpo-price">%s</span>', $product->get_sale_price() ),
			'{price_suffix}'  => sprintf( '<span class="wpo-price-suffix">%s</span>', $product->get_price_suffix() ),
		];

		foreach ( $placeholders as $placeholder => $replacement ) {
			$price_template = str_replace( $placeholder, $replacement, $price_template );
		}
	...
	/**
	 * Retrieves the price display format based on the groups assigned to the product.
	 *
	 * @param array $groups
	 * @return string The price display format.
	 */
	public static function get_price_display_format( $groups ): string {
		foreach ( $groups as $group ) {
			$group_settings      = $group->settings ?? [];
			$group_price_display = $group_settings['price_display'] ?? 'default';

			if ( $group_price_display === 'hide' ) {
				return '';
			}

			if ( $group_price_display === 'extend' ) {
				if ( empty( $prefix ) ) {
					$prefix = $group_settings['price_display_prefix'] ?? '';
				}

				if ( empty( $suffix ) ) {
					$suffix = $group_settings['price_display_suffix'] ?? '';
				}
		...
		if ( ! empty( $prefix ) || ! empty( $suffix ) ) {
			return sprintf(
				'<span class="wpo-price-display"><span class="wpo-price-display-prefix">%1$s</span>%2$s<span class="wpo-price-display-suffix">%3$s</span></span>',
				$prefix,
				'%s',
				$suffix
			);
	...
	public static function get_product_price_exclusion_status( $product ): bool {
		...
		$formula_options = array_reduce(
			$groups,
			function ( $carry, $group ) use ( $groups ) {
				$group_formula_options = array_filter(
					$group->options ?? [],
					function ( $option ) {
				...
				return array_merge( $carry, $group_formula_options );
			},
			[]
		);

		if ( empty( $formula_options ) ) {
			return false;
		}
		...
		$exclude_product_price = array_filter(
			$formula_options,
			function ( $option ) {
				return $option->settings['exclude_product_price'] ?? false;
			}
		);
