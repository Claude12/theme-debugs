 ...
 * Panel components
 */
import PriceDisplayRow from '../components/tables/rows/group-price-display';
...
const GroupForm = ( { editMode, group } ) => {
	...
	const onSubmit = async ( data ) => {
		...
		if ( data ) {
			if (
				data?.visibility?.endsWith( 'specific' ) &&
				...
				setNotification(
					'error',
					__( 'Please select one or more products, categories, or user roles.', 'woocommerce-product-options' ),
					false,
					true
				);
									...
									render={ ( { field } ) => (
										<RadioControl
											...
											options={ [
												...
												{
													label: __(
														'Show on specific categories, products, or user roles',
														'woocommerce-product-options'
													),
						...
						{ visibilityInput?.endsWith( 'specific' ) && (
							...
							>
								<div className="wpo-split-row">
									...
									<Controller
										...
										render={ ( { field } ) => (
											<UserSelect
												prefilled={ group?.user_roles ?? false }
												onChange={ ( value ) => field.onChange( value ) }
												value={ field?.value ?? [] }
											/>
										) }
							...
							<FormRow name={ 'exclusions' } label={ __( 'Exclusions', 'woocommerce-product-options' ) }>
								<div className="wpo-split-row">
									...
									<Controller
										control={ formMethods.control }
										name={ 'exclude_user_roles' }
										render={ ( { field } ) => (
											<UserSelect
												prefilled={ group?.exclude_user_roles ?? false }
												onChange={ ( value ) => field.onChange( value ) }
												value={ field?.value ?? [] }
											/>
										) }
							...
							<PriceDisplayRow group={ group } formMethods={ formMethods } />
