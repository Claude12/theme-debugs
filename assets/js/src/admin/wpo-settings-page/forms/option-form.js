 ...
 * Panel components
 */
import ChoiceSettingsRow from '../components/tables/rows/option-choice-settings';
...
const OptionForm = ( { formMethods, index, option } ) => {
	 ...
	 */
	useEffect( () => {
		if ( prevChoiceTypeRef.current === 'variation_attributes' && prevChoiceTypeRef.current !== choiceType ) {
		...
		<>
			<table className="option-form-table widefat fixed">
				<tbody>
					...
					{ supportsVariationAttributes() && (
						...
						>
							<Controller
								...
								name={ `options.${ index }.settings.choice_type` }
								render={ ( { field } ) => (
									<div className="wpo-choice-type-selector">
										{/* Custom choices radio */}
										<label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
											<input
												type="radio"
												checked={ field?.value !== 'variation_attributes' }
												onChange={ () => field.onChange( 'custom' ) }
											/>
											{ __( 'Create custom choices', 'woocommerce-product-options' ) }
										</label>
										
										{/* Conditional checkbox - only shows when custom is selected */}
										{ field?.value !== 'variation_attributes' && (
											<div className="wpo-sub-checkbox">
												<CheckboxControl
													isClassicStyle
													checked={ field?.value === 'custom_with_qty' }
													label={ __( 'Display a quantity picker alongside each choice', 'woocommerce-product-options' ) }
													onChange={ ( checked ) => field.onChange( checked ? 'custom_with_qty' : 'custom' ) }
												/>
											</div>
										) }
										
										{/* Variation attributes radio */}
										<label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
											<input
												type="radio"
												checked={ field?.value === 'variation_attributes' }
												onChange={ () => field.onChange( 'variation_attributes' ) }
											/>
											{ __( 'Create choices using existing variation attributes', 'woocommerce-product-options' ) }
										</label>
									</div>
								) }
							/>
					...
					{ advancedSettings && ! singleChoice && choiceType !== 'variation_attributes' && (
						<ChoiceSettingsRow
							index={ index }
							formMethods={ formMethods }
							option={ option }
					...
					{ ! [ 'customer_price', 'price_formula', 'product', 'wysiwyg', 'html' ].includes( optionType ) && advancedSettings && (
						<OptionFormRow
							name="settings.hide_prices"
							label={ __( 'Hide prices', 'woocommerce-product-options' ) }
						>
							<Controller
								control={ formMethods.control }
								name={ `options.${ index }.settings.hide_prices` }
								render={ ( { field } ) => (
									<CheckboxControl
										label={ __( 'Do not display option prices on the product page', 'woocommerce-product-options' ) }
										checked={ [ '1', 1, true ].includes( field?.value ) }
										onChange={ ( value ) => field.onChange( value ) }
										isClassicStyle={ true }
									/>
