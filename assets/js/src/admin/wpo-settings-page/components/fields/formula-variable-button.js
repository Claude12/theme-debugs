...
const FormulaVariableButton = ( { option, insertVariable } ) => {
	...
	const choiceButton = () => {
		...
		const hasQuantity = option?.settings?.choice_type === 'custom_with_qty';
					...
					...option.choices.map( ( choice ) => {
						...
						return {
							...
							subItems: [
								...
								hasQuantity && {
									key: `${ option.id }.choices.${ choice.id }.quantity`,
									value: choice.id,
									onClick: () =>
										insertVariable( `[${ optionName }.choices.${ sanitizedLabel }.quantity]` ),
									label: __( 'Quantity', 'woocommerce-product-options' ),
								},
							].filter( Boolean ),
						};
					} ),
		...
		items.push(
			...
			! singleChoice && hasQuantity && {
				key: `${ option.id }.totalQty`,
				value: option.id,
				onClick: () => insertVariable( `[${ optionName }.totalQty]` ),
				label: __( 'Total quantity', 'woocommerce-product-options' ),
			},
			singleChoice && {
			...
			singleChoice && hasQuantity && {
				key: `${ option.id }.quantity`,
				value: option.id,
				onClick: () => insertVariable( `[${ optionName }.quantity]` ),
				label: __( 'Selected quantity', 'woocommerce-product-options' ),
			}
		);
