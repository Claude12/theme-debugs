...
const OptionChoicesRepeater = ( {
	...
	const choiceType = option?.settings?.choice_type;
	...
	const choiceQtyMax = parseInt( option?.settings?.choice_qty?.max || '' );
	...
	const getColumnWidths = () => {
		...
		if ( ! singleChoice && columnState.selected && choiceType !== 'custom_with_qty' ) {
			colWidths.push( widths.selected );
		}

		if ( ! singleChoice && choiceType === 'custom_with_qty' ) {
			colWidths.push( widths.quantity );
		}
	...
	const handleChoiceChange = ( changeValue, key, choiceId ) => {
		const newChoices = choices.map( ( choice ) => {
			if ( key === 'default_qty'
				&& ( [ 'radio', 'color_swatches', 'dropdown' ].includes( optionType )
					|| choiceQtyMax === 1 )
			) {
				// If this is a radio-like option, reset default_qty for all choices
				// The one being changed will be set at the end of this function
				choice = { ...choice, 'default_qty': '' };
			}
			...
			<table className={ containerClasses } style={ { gridTemplateColumns: getColumnWidths() } }>
				<thead className="choice-headers">
					<tr>
						...
						{ ! singleChoice && choiceType === 'custom_with_qty' && (
							<th className={ 'option-choices-repeater-quantity-col' } colSpan={ 1 }>
								{ __( 'Quantity', 'woocommerce-product-options' ) }
								<WCTableTooltip
									tooltip={ __(
										'The default quantity preset for each choice.'
									) }
								/>
							</th>
						) }

						{ ! singleChoice && columnState.selected && choiceType !== 'custom_with_qty' && (
							...
							<>
								...
								<th className={ 'option-setting-repeater-clone-col' } colSpan={ 1 }>
									<CustomColumnPopover
										...
										hidden={ {
											selected: choiceType === 'custom_with_qty',
										} }
										...
										<Draggable key={ choice.id } draggableId={ choice.id } index={ index }>
											{ ( draggableProvided, draggableSnapshot ) => (
												...
												>
													...
													{ ! singleChoice && choiceType === 'custom_with_qty' && (
														<td colSpan={ 1 } className="quantity-wrap">
															<label
																htmlFor={ `wpo-choice-${ choice.id }-quantity` }
																className="compact-label"
															>
																{ __( 'Quantity', 'woocommerce-product-options' ) }
															</label>
															<input
																id={ `wpo-choice-${ choice.id }-quantity` }
																type="number"
																min="0"
																value={ choice?.default_qty ?? defaultChoice.default_qty }
																onChange={ ( event ) => handleChoiceChange( event.target.value, 'default_qty', choice.id ) }
															/>
														</td>
													) }
