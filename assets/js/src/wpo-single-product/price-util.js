...
export const calculateOptionPrice = ( field, price, variation = {} ) => {
	...
	const productPrice = ( variation?.price ?? productVariables.price );

	switch ( price.type ) {
		case 'flat_fee':
			const addonFactor = price.qtyPicker * ( wpoSettings.addons_use_cart_quantity ? quantity : 1 );
			calculatedPrice = price.amount * ( field.element.dataset?.parentType === 'product' ? addonFactor : 1 );
			break;
		case 'quantity_based':
			calculatedPrice = price.qtyPicker * price.amount * quantity;
			break;
		case 'percentage_inc':
			calculatedPrice = price.qtyPicker * productPrice * ( price.amount / 100 ) * quantity;
			break;
		case 'percentage_dec':
			calculatedPrice = -( price.qtyPicker * productPrice * ( price.amount / 100 ) ) * quantity;
			break;
		case 'char_count':
			calculatedPrice = price.amount * getCharCount( field ) * quantity;
			break;
		case 'file_count':
			calculatedPrice = price.amount * getFileCount( field ) * quantity;
			break;
		case 'price_formula':
			calculatedPrice = price.amount * quantity;
			break;
		default:
			const callback = customPriceTypes[ price.type ]?.callback;
			if ( typeof callback === 'function' ) {
				calculatedPrice = callback( field, price, productPrice, variation );
			}
			break;
	}
		...
		Array.from( field.element.querySelectorAll( 'input[type="checkbox"]' ) ).forEach( ( checkbox ) => {
			if ( checkbox.checked ) {
				chosenPrices.push( {
					...
					qtyPicker: parseInt( checkbox.closest( 'li' )?.querySelector( 'input.wpo-qty-picker[type="number"]' )?.value ?? 1 ),
				} );
			}
		} );
	}
		...
		Array.from( field.element.querySelectorAll( 'input[type="radio"]' ) ).forEach( ( radio ) => {
			if ( radio.checked ) {
				chosenPrices.push( {
					...
					qtyPicker: parseInt( radio.closest( '.wpo-field' )?.querySelector( 'input.wpo-qty-picker[type="number"]' )?.value ?? 1 ),
				} );
			}
		} );
	}
	...
	if ( isFieldType( field, 'dropdown' ) ) {
		...
		if ( selectElement?.selectedOptions?.length > 0 ) {
			...
			selectedOptions.forEach( ( selectedOption ) => {
				const optionId = `${selectElement.id}-${selectedOptions.indexOf( selectedOption )}`;
				chosenPrices.push( {
					id: optionId,
					type: selectedOption.dataset.priceType,
					amount: parseFloat( selectedOption.dataset.priceAmount ),
					qtyPicker: parseInt( selectedOption.closest( '.wpo-field' )?.querySelector( 'input.wpo-qty-picker[type="number"]' )?.value ?? 1 ),
				} );
			} );
		}
	}
