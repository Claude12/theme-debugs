...
export const processFormulaVariable = ( variable, form ) => {
	...
	let qtyPicker = inputField?.closest( 'li' )?.querySelector( 'input.wpo-qty-picker[type="number"]' );

	if ( ! qtyPicker ) {
		// if the quantity picker is not inside a list item, it means it's a single-selection field
		// so we look for the qty picker inside the whole field wrapper
		qtyPicker = inputField?.closest( '.wpo-field' )?.querySelector( 'input.wpo-qty-picker[type="number"]' );
	}	
	...
	const getQuantityValue = ( element ) => {
		let qty = 0;
		let elQtyPicker = element.closest( 'li' )?.querySelector( 'input.wpo-qty-picker[type="number"]' );

		if ( ! elQtyPicker ) {
			// if the quantity picker is not inside a list item, it means it's a single-selection field
			// so we look for the qty picker inside the whole field wrapper
			elQtyPicker = inputField?.closest( '.wpo-field' )?.querySelector( 'input.wpo-qty-picker[type="number"]' );
		}

		if ( elQtyPicker && ! isNaN( parseInt( elQtyPicker.value ) ) ) {
			qty = parseInt( elQtyPicker.value );
		}
	...
	switch ( variable.type ) {
		...
		case 'text_labels_option':
			...
			choiceElements.forEach( ( element, index ) => {
				...
				choiceVariable.choices[ `choice${index}` ] = {
					...
					value: isChecked ? evaluateFieldValue( element.dataset?.formulaValue || 0 ) : 0,
					quantity: isChecked ? evaluateFieldValue( getQuantityValue( element ) ) : 0,
				};
			} );
			
			const totalQty = Array.from( selectedChoices ).reduce( ( total, element ) => {
				return total + getQuantityValue( element );
			}, 0 );
			...
			return {
				[ variableName ]: {
					...
					totalqty: evaluateFieldValue( totalQty ),
					...
					value: evaluateFieldValue(
						inputField.querySelector( selectedInputType )?.dataset?.formulaValue || 0
					),
					quantity: evaluateFieldValue(
						qtyPicker ? parseInt( qtyPicker.value ) : 0
					),
				},
			};
		...
		case 'product_option':
			...
			const productElements = Array.from( inputField?.querySelectorAll( inputType ) )?.filter( ( element ) => element.value && ! element.disabled );
