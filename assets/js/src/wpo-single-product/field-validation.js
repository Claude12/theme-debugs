...
const fieldValidation = ( addToCartForm, addtoCartButton = null ) => {
	...
	const allFields = [];
	const isVariationForm = [ 'variations_form', 'wpt_variations_form' ].some( ( className ) => form.classList.contains( className ) );
	...
	function init() {
		...
		allFields.push(
			...Array.from( form.querySelectorAll( '.wpo-field' ), ( element ) => {
				const inputElements = element.querySelectorAll( getFieldInputType( element.dataset.type ) );
				const qtyPickers = element.querySelectorAll( 'input.wpo-qty-picker[type="number"]' );
				...
				return {
					...
					qtyTotalMin: element.dataset?.qtyTotalMin ? parseInt( element.dataset.qtyTotalMin ) : null,
					qtyTotalMax: element.dataset?.qtyTotalMax ? parseInt( element.dataset.qtyTotalMax ) : null,
	...
	function bindEvents() {
		allFields.forEach( ( field ) => {
			...
			[ ...field.inputElements, ...field.qtyPickers ].forEach( ( inputElement ) => {
		...
		allFields.forEach( ( field ) => {
			field.qtyPickers.forEach( ( qtyPicker ) => {
				if ( qtyPicker.closest( '.wpo-product-option__list-product' ) ) {
					// Qty picker is in a product list, no need to bind quantity change event
					return;
				}

				qtyPicker.addEventListener( 'change', () => {
					emitFieldEvent( 'quantity_change', field );
					reportFieldValidityInHTML( field );
					submitButton.classList.toggle( 'disabled', ! checkFormValidity( false ) );
				} );
			} );
	...
	function reportFieldValidityInHTML( field, notify = true ) {
		...
		if ( ! callback( field ) ) {
			if ( notify ) {
				...
				errorMessage.innerHTML = firstInput?.validationMessage;
				...
				if ( isFieldType( field, [ 'datepicker', 'file_upload' ] ) ) {
					errorMessage.innerHTML = firstInput?.dataset?.validationMessage ?? '';
				}
				// add to the field
	...
	function checkFormValidity( notify = true ) {
		...
		form.dispatchEvent( new CustomEvent( 'wpo:recalculate', { detail: { form } } ) );
	...
	function handleCheckboxLimitsValidation( field ) {
		...
		const qtyLimitsValidity = checkQtyLimitsValidity( field );
		...
		if ( ( isFieldRequired( field ) || checkedInputs.length > 0 ) && minQty && checkedInputs.length < minQty ) {
			...
			return false;
		} else if ( qtyLimitsValidity !== '' ) {
			// Multi-selection fields also require quantity limits validation
			firstInput?.setCustomValidity( qtyLimitsValidity );
			return false;
		}
	...
	function handleRadioRequiredValidation( field ) {
		...
		if ( selectedInputs.length === 0 && isFieldRequired( field ) ) {
			...
			// Radio groups also require quantity limits validation
			firstInput?.setCustomValidity( qtyLimitsValidity );
			return false;
		}
	...
	function handleSelectRequiredValidation( field ) {
		...
		if ( ( isFieldRequired( field ) || selectedOptions.length > 0 ) && minQty && selectedOptions.length < minQty ) {
			const minValidationMessage = minQty === 1
				? __(
						'Please select an option.',
						'woocommerce-product-options'
				)
			...
			firstInput?.setCustomValidity( minValidationMessage );
			return false;
		} else if ( maxQty === 1 && selectedOptions.length > maxQty ) {
	...
	function checkQtyLimitsValidity( field ) {
		const fieldQtyTotal = Array.from( field.qtyPickers ).reduce( ( total, qp ) => total + parseInt( qp.value ), 0 );

		if ( ! isFieldRequired( field ) && fieldQtyTotal === 0 ) {
			return '';
		}

		const optionName = field.element.querySelector( '.wpo-option-name .wpo-option-name__text' )?.innerText || '';

		const qtyPickersValidityMessage = Array.from( field.qtyPickers ).reduce( ( msg, qtyPicker ) => {
			const value = parseInt( qtyPicker.value || 0 );
			const min = parseInt( qtyPicker.min || 0 );
			const max = parseInt( qtyPicker.max || 0 );
			let choiceLabel = qtyPicker.closest( 'li' )?.querySelector('.wpo-choice-label')?.innerText || optionName;
			let newMsg = '';

			if ( ( wpoSettings.show_one_error_per_field ?? true ) && msg.length > 0 ) {
				return msg;
			}
			...
			if ( min && value < min ) {
				newMsg = sprintf(
					...
					__(
						'The quantity of %s must be at least %d.',
						'woocommerce-product-options'
					),
					choiceLabel,
					min
			...
			if ( max && value > max ) {
				newMsg = sprintf(
					...
					__(
						'The quantity of %s must not exceed %d.',
						'woocommerce-product-options'
					),
					choiceLabel,
					max
		...
		if ( qtyPickersValidityMessage.length > 0 ) {
			return qtyPickersValidityMessage.join( '<br>' );
		}

		if ( field.qtyTotalMin && fieldQtyTotal < field.qtyTotalMin ) {
			return sprintf(
				/* translators: %d: minimum total quantity */
				__(
					'The total quantity must be at least %d across all selected choices.',
					'woocommerce-product-options'
				),
				field.qtyTotalMin
			);
		...
		if ( field.qtyTotalMax && fieldQtyTotal > field.qtyTotalMax ) {
			return sprintf(
				/* translators: %d: maximum total quantity */
				__(
					'The total quantity must not exceed %d across all selected choices.',
					'woocommerce-product-options'
				),
				field.qtyTotalMax
			);
