...
const priceCalculator = ( addToCartForm ) => {
	...
	const init = () => {
		...
		checkboxFields = Array.from( form.querySelectorAll( '.wpo-field' ) )
			...
			.map( ( field ) => {
				return {
					element: field,
					inputElements: field.querySelectorAll( 'input:not(.wpo-qty-picker)' ),
					qtyPicker: field.querySelectorAll( 'input.wpo-qty-picker' ),
					type: field.dataset?.type,
					groupId: field.dataset?.groupId ? parseInt( field.dataset.groupId ) : null,
					optionId: field.dataset?.optionId ? parseInt( field.dataset.optionId ) : null,
				};
	...
	const addOptionPrice = ( field, price ) => {
		const optionPrice = calculateOptionPrice( field, price );
	 ...
	 */
	const calculatePricing = ( cartForm ) => {
		const showInvalidFormTotal = wpoSettings?.show_invalid_form_total;
		const validFormMultiplier = ( showInvalidFormTotal || cartForm.checkValidity() ) ? 1 : 0;

		const optionsPrice = subTotals.reduce( ( total, subTotal ) => {
			return total + subTotal.price * validFormMultiplier;
		}, 0 );
		...
		cartForm.dispatchEvent( new CustomEvent( 'wpo:recalculated', { detail: { form: cartForm, total: totalsContainer?.dataset?.totalPrice } } ) )
	...
	const setPricingInHtml = ( totalPrice, optionsPrice ) => {
		...
		if ( totalPrice === 0 || optionsPrice === 0 && productVariables.quantity === 1 ) {
